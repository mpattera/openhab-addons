...
/**
 * Receives complete state snapshots for a zone.
 *
 * @author Michele Pattera - Initial contribution
 */
@FunctionalInterface
@NonNullByDefault
public interface ZoneStateListener {

    /**
     * Called when the panel has a new state for the registered zone.
     *
     * @param state the complete state snapshot
     */
    void onZoneStateUpdated(ZoneSnapshot state);
     ...
     */
    default void onZoneStateInvalidated() {
    }
}
